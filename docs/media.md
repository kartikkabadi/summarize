...
- Loom share/embed video URLs (`loom.com/share/<id>`, `loom.com/embed/<id>`) route through the generic yt-dlp + transcription path in auto and prefer modes. This reuses yt-dlp’s Loom extractor (no bespoke Loom API). With `--video-mode transcript` (prefer), Loom HTML may be fetched only for caption-track discovery; transcript failures reject with the normal yt-dlp/provider diagnostics instead of returning landing-page HTML. In auto mode, Loom transcription is still attempted, but page content may be returned if transcription is unavailable. Loom is not included in the shared `shouldPreferUrlMode` hard-switch used by Chrome/daemon auto routing, because guarded daemon URL fetches intentionally cannot pass unresolved Loom URLs to yt-dlp; browser-supplied page text remains usable unless the user explicitly selects video mode.
