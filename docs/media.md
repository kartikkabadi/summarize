...
- Loom share/embed video URLs (`loom.com/share/<id>`, `loom.com/embed/<id>`) route through the generic yt-dlp + transcription path in auto and prefer modes. This reuses yt-dlp’s Loom extractor (no bespoke Loom API). With `--video-mode transcript` (prefer), transcript failures reject with the normal yt-dlp/provider diagnostics instead of returning Loom landing-page HTML. In auto mode, Loom transcription is still attempted, but page content may be returned if transcription is unavailable. Guarded daemon URL fetches still do not pass arbitrary Loom URLs to yt-dlp unless the existing external-downloader policy explicitly allows it.
