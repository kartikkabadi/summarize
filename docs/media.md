...
- Loom share/embed video URLs (`loom.com/share/<id>`, `loom.com/embed/<id>`) route through the generic yt-dlp + transcription path in auto and prefer modes. This reuses yt-dlp’s Loom extractor (no bespoke Loom API). Private or access-restricted Loom videos fail with the normal yt-dlp/provider errors rather than a page-summary transcript. Guarded daemon URL fetches still do not pass arbitrary Loom URLs to yt-dlp unless the existing external-downloader policy explicitly allows it.
