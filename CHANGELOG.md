...
- Loom: recognize share/embed video URLs and route them through the generic yt-dlp transcription path with audio-capable format selection; explicit `--video-mode transcript` failures reject instead of returning landing-page HTML, while prefer-mode still discovers embedded captions from Loom HTML without substituting embedded YouTube URLs. Loom is not hard-preferred in shared Chrome/daemon URL routing so guarded environments keep using supplied page text (#353).
